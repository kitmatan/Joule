                ...
                VStack(alignment: .leading, spacing: 16) {
                    ...
                    VStack(spacing: 0) {
                        if let mileage = session.mileage {
                            DetailRow(title: "Mileage", value: String(format: "%.0f km", mileage), icon: "speedometer")
                        ...
                        if let startR = session.startRange, let endR = session.endRange {
                            DetailRow(title: "Est. Range", value: String(format: "%.0f km → %.0f km", startR, endR), icon: "car")
                            Divider().padding(.leading, 44)
                        }
                        ...
                        if efficiency > 0 {
                            Divider().padding(.leading, 44)
                            DetailRow(title: "Efficiency", value: String(format: "฿%.2f / kWh", efficiency), icon: "leaf")
                        }
                    ...
                    VStack(alignment: .leading, spacing: 16) {
                        ...
                        VStack(spacing: 0) {
                            if session.chargingFee > 0 {
                                DetailRow(title: "Charging Fee", value: session.chargingFee.formatted(.currency(code: "THB").presentation(.narrow)), icon: "bolt")
                            }
                            if session.bookingFee > 0 {
                                Divider().padding(.leading, 44)
                                DetailRow(title: "Booking Fee", value: session.bookingFee.formatted(.currency(code: "THB").presentation(.narrow)), icon: "calendar.badge.clock")
                            }
                            if session.overtimeFee > 0 {
                                Divider().padding(.leading, 44)
                                DetailRow(title: "Overtime Fee", value: session.overtimeFee.formatted(.currency(code: "THB").presentation(.narrow)), icon: "clock.badge.exclamationmark")
                            }
